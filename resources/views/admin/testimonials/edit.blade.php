...
@section('content')
    <div class="max-w-4xl space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
        ...
        <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" class="space-y-10 pb-20">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-4">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Pekerjaan /
                            Kota</label>
                        <input type="text" name="occupation" value="{{ old('occupation', $testimonial->occupation) }}"
                    ...
                    <div class="space-y-4 md:col-span-2">
                        ...
                        <textarea name="content" rows="6"
                            ...
                            placeholder="Tuliskan pengalaman positif pelanggan di sini..." required>{{ old('content', $testimonial->content) }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-[10px] font-black uppercase tracking-widest ml-4 mt-2">
                                {{ $message }}</p>
                        @enderror
            ...
            <button type="submit"
                class="w-full py-8 bg-indigo-500 text-white rounded-[2.5rem] font-black text-xl tracking-widest uppercase hover:bg-slate-900 transition-all shadow-xl shadow-indigo-500/20 active:scale-95 duration-500">
                Perbarui Testimoni
            </button>
        </form>
    </div>
