...
@section('title', 'Edit Data Motor: ' . $motorcycle->name)
    ...
    <div class="max-w-4xl space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
        ...
        <form action="{{ route('admin.motorcycles.update', $motorcycle) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-4">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Nama
                            Motor</label>
                        <input type="text" name="name" value="{{ old('name', $motorcycle->name) }}"
                    ...
                    <div class="space-y-4 md:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                            ...
                            placeholder="Jelaskan detail dan kelebihan motor ini..." required>{{ old('description', $motorcycle->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-[10px] font-black uppercase tracking-widest ml-4 mt-2">
                                {{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-4">
                        ...
                        <input type="number" name="price_per_day"
                            value="{{ old('price_per_day', (int)$motorcycle->price_per_day) }}"
                    ...
                    <div class="space-y-4">
                        ...
                        <input type="number" name="price_per_month"
                            value="{{ old('price_per_month', $motorcycle->price_per_month ? (int)$motorcycle->price_per_month : '') }}"
                    ...
                    <div class="space-y-4">
                        ...
                        <select name="status"
                            ...
                            required>
                            <option value="active" {{ old('status', $motorcycle->status) == 'active' ? 'selected' : '' }}>
                                Aktif Tersedia</option>
                            <option value="inactive"
                                {{ old('status', $motorcycle->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif /
                                Maintenance</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-[10px] font-black uppercase tracking-widest ml-4 mt-2">
            ...
            <div class="bg-white rounded-[3rem] p-12 shadow-sm border border-gray-100">
                ...
                <!-- Images Gallery -->
                <div class="mb-12">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4 block mb-6">Foto Saat Ini</label>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                        @forelse ($motorcycle->getMedia('motorcycles') as $media)
                            <div class="relative group rounded-2xl overflow-hidden aspect-square border border-gray-100 shadow-sm transition-transform hover:scale-105">
                                <img src="{{ $media->getUrl() }}" class="w-full h-full object-cover">
                            </div>
                        @empty
                            <div class="col-span-full py-10 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 text-center">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Belum ada foto</p>
                            </div>
                        @endforelse
                    </div>
                </div>
                ...
                <!-- Videos Gallery -->
                <div class="mb-12">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4 block mb-6">Video Saat Ini</label>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @forelse ($motorcycle->getMedia('videos') as $media)
                            <div class="relative rounded-2xl overflow-hidden aspect-video border border-gray-100 shadow-sm bg-slate-900">
                                <video src="{{ $media->getUrl() }}" class="w-full h-full object-cover" controls></video>
                            </div>
                        @empty
                            <div class="col-span-full py-10 bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 text-center">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Belum ada video</p>
                            </div>
                        @endforelse
                    </div>
                </div>
                ...
                <!-- Upload Section -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-10 border-t border-gray-100">
                    <div class="space-y-6">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4 block">Tambah Foto Baru</label>
                        <div class="relative group border-2 border-dashed border-gray-200 rounded-[2.5rem] p-10 hover:border-blue-500 transition-all bg-gray-50/50 hover:bg-blue-50/10 cursor-pointer text-center">
                            ...
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-[10px] font-black text-slate-800 uppercase tracking-widest">Unggah Foto</p>
                                <p class="text-[9px] font-bold text-slate-400 italic mt-1">JPG/PNG (Max 2MB)</p>
                            </div>
                        </div>
                        @error('images.*')
                    ...
                    <div class="space-y-6">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4 block">Tambah Video Baru</label>
                        <div class="relative group border-2 border-dashed border-gray-200 rounded-[2.5rem] p-10 hover:border-blue-500 transition-all bg-gray-50/50 hover:bg-blue-50/10 cursor-pointer text-center">
                            ...
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-[10px] font-black text-slate-800 uppercase tracking-widest">Unggah Video</p>
                                <p class="text-[9px] font-bold text-slate-400 italic mt-1">MP4/MOV (Max 20MB)</p>
                            </div>
                        </div>
                        @error('videos.*')
                            <p class="text-red-500 text-[10px] font-black uppercase tracking-widest ml-4 mt-2">{{ $message }}</p>
            ...
            <button type="submit"
                class="w-full py-8 bg-blue-500 text-white rounded-[2.5rem] font-black text-xl tracking-widest uppercase hover:bg-slate-900 transition-all shadow-xl shadow-blue-500/20 active:scale-95 duration-500">
                Perbarui Data Motor
            </button>
        </form>
    </div>
