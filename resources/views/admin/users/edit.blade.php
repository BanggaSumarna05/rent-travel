...
@section('content')
    <div class="max-w-4xl space-y-10 animate-fade-in">
        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-10 pb-20">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-[3rem] p-10 lg:p-14 shadow-sm border border-slate-100 relative overflow-hidden group">
                ...
                <h3 class="text-xl font-black text-slate-900 mb-12 flex items-center gap-5 relative z-10 uppercase tracking-tighter italic">
                    ...
                    Edit Profil <span class="gold-gradient-text ml-2">Admin</span>
                </h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
                    ...
                    <div class="md:col-span-2 p-8 bg-slate-50 rounded-[2rem] border border-slate-100">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 italic">Kosongkan jika tidak ingin mengubah password</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-4">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] ml-2">Password Baru</label>
                                <input type="password" name="password"
                                    class="w-full px-8 py-5 bg-white border-2 border-slate-100 rounded-[1.5rem] focus:outline-none focus:ring-0 focus:border-amber-500/50 transition-all font-bold text-sm tracking-tight @error('password') border-red-500 @enderror"
                            ...
                            <div class="space-y-4">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] ml-2">Konfirmasi Password Baru</label>
                                <input type="password" name="password_confirmation"
                                    class="w-full px-8 py-5 bg-white border-2 border-slate-100 rounded-[1.5rem] focus:outline-none focus:ring-0 focus:border-amber-500/50 transition-all font-bold text-sm tracking-tight"
                    ...
                    <div class="flex items-center gap-6 p-8 bg-slate-900 rounded-[2rem] border border-white/5 shadow-xl shadow-slate-950/20 md:col-span-2">
                        <div class="relative inline-flex items-center cursor-pointer scale-110">
                            ...
                            <input type="checkbox" name="is_admin" value="1" id="is_admin"
                                class="sr-only peer" {{ old('is_admin', $user->is_admin) ? 'checked' : '' }}>
                            <div
            ...
            <button type="submit"
                ...
                <span class="relative z-10">Perbarui Data Admin</span>
