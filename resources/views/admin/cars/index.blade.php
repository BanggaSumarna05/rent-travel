...
@section('content')
    <div class="space-y-8 animate-fade-in">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center shadow-lg shadow-amber-500/10 border border-white/5">
                    <i class="fa-solid fa-car text-xl text-[#D4AF37]"></i>
                </div>
                <div>
                    <h2 class="text-2xl md:text-3xl font-black text-slate-900 tracking-tight leading-tight uppercase">
                        Daftar Armada <span class="gold-gradient-text italic">Mobil</span>
                    </h2>
                    <p class="text-slate-500 font-semibold text-sm italic">
                        Kelola data unit mobil dan status operasional.
                    </p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                ...
                <div class="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                        ...
                        <div class="text-xl font-black text-slate-900 tracking-tight">{{ $totalCars }} <span class="text-xs text-slate-400">Unit</span></div>
                    </div>
                </div>
            </div>
            <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
            ...
            <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                ...
                <div class="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                        <div class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-0.5">Unit Unggulan</div>
                        <div class="text-xl font-black text-slate-900 tracking-tight">{{ $totalFeatured }} <span class="text-xs text-slate-400">Unit</span></div>
                    </div>
                </div>
            </div>
            <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                <form action="{{ route('admin.cars.index') }}" method="GET" class="relative items-center flex h-full">
        ...
        <div class="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-400">
                            ...
                            <th class="px-8 py-8 text-[10px] font-black uppercase tracking-[0.2em]">Spesifikasi Teknis</th>
                            ...
                            <tr class="group hover:bg-slate-50/30 transition-all duration-500">
                                <td class="px-8 py-10">
                                    <div class="flex items-center gap-6">
                                        ...
                                        <div>
                                            <div class="text-xl font-black text-slate-900 tracking-tight leading-none mb-2 uppercase">{{ $car->name }}</div>
                                            <div class="flex items-center gap-3">
                                                <span class="text-[9px] font-black gold-gradient-text uppercase tracking-[0.2em] bg-amber-50 px-2 py-0.5 rounded-md border border-amber-100/50">{{ $car->brand }}</span>
                                                <div class="w-1.5 h-1.5 rounded-full bg-slate-200"></div>
                                                <span class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] italic">{{ str_replace('_', ' ', $car->category) }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-10">
                                    <div class="flex flex-col gap-4">
                                        <div class="flex items-center gap-3 text-[11px] font-black text-slate-600">
                                            ...
                                            <span class="uppercase italic break-all">{{ $car->transmission }}</span>
                                        </div>
                                        <div class="flex items-center gap-3 text-[11px] font-black text-slate-600">
                                            <div class="w-9 h-9 bg-slate-50 rounded-xl flex items-center justify-center border border-slate-100 group-hover:bg-white transition-all shadow-sm">
                                                <i class="fa-solid fa-users text-amber-500"></i>
                                            </div>
                                            <span class="uppercase italic break-all">{{ $car->passenger_capacity }} Penumpang</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-10">
                                ...
                                <td class="px-8 py-10">
                                    <div class="flex flex-col gap-3 items-start">
                                        ...
                                        @endif
                                        @if ($car->is_featured)
                                            <span class="px-5 py-2.5 bg-slate-900 text-amber-400 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] border border-white/10 flex items-center gap-3 shadow-xl shadow-slate-950/20">
                                ...
                                <td class="px-8 py-10">
                                    <div class="flex justify-end gap-4 overflow-hidden">
                                        ...
                                        <form action="{{ route('admin.cars.destroy', $car) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus unit ini?')">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                ...
                @foreach ($cars as $car)
                    <div class="p-8 hover:bg-slate-50/50 transition-all duration-300">
                        <div class="flex items-start gap-6 mb-8">
                            ...
                            <div class="flex-1 min-w-0 pt-2">
                                ...
                                <div class="flex items-center gap-2 mb-4">
                                    <span class="text-[8px] font-black gold-gradient-text uppercase tracking-widest bg-amber-50 px-2 py-0.5 rounded border border-amber-100/50">{{ $car->brand }}</span>
                                    <span class="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] italic">{{ str_replace('_', ' ', $car->category) }}</span>
                                </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-8">
                            <div class="flex items-center gap-3 text-[10px] font-black text-slate-600 bg-slate-50 px-5 py-4 rounded-[1.5rem] border border-slate-100 shadow-sm">
                                <i class="fa-solid fa-gears text-amber-500"></i>
                                <span class="uppercase italic truncate">{{ $car->transmission }}</span>
                            </div>
                            <div class="flex items-center gap-3 text-[10px] font-black text-slate-600 bg-slate-50 px-5 py-4 rounded-[1.5rem] border border-slate-100 shadow-sm">
                                <i class="fa-solid fa-users text-amber-500"></i>
                                <span class="uppercase italic truncate">{{ $car->passenger_capacity }} Kursi</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between gap-6">
                            <div class="flex flex-wrap gap-3">
                                ...
                                @endif
                                @if ($car->is_featured)
                                    <span class="px-5 py-2.5 bg-slate-900 text-amber-400 rounded-full text-[9px] font-black uppercase tracking-widest shadow-xl shadow-slate-950/20 flex items-center gap-2 border border-white/5">
                                        <i class="fa-solid fa-crown text-amber-500"></i>
                                        Utama
                                    </span>
                                @endif
                            ...
                            <div class="flex gap-4">
                                <a href="{{ route('admin.cars.edit', $car) }}" class="p-4 bg-slate-900 text-amber-400 rounded-2xl shadow-xl shadow-slate-950/20 border border-white/5">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                                <form action="{{ route('admin.cars.destroy', $car) }}" method="POST" onsubmit="return confirm('Hapus unit ini?')">
                                    @csrf @method('DELETE')
            ...
            <!-- Empty State -->
            @if($cars->isEmpty())
                <div class="py-40 px-10 text-center bg-slate-50/20 relative overflow-hidden group">
                    ...
                    <div class="w-32 h-32 bg-white rounded-[3rem] shadow-2xl shadow-slate-200/50 border border-slate-100 flex items-center justify-center mx-auto mb-10 relative z-10 transition-transform duration-700 group-hover:scale-110 group-hover:rotate-6">
                         <i class="fa-solid fa-car-rear text-5xl text-slate-200 group-hover:text-amber-200 transition-colors duration-700"></i>
                    </div>
                    <div class="relative z-10">
                        <h3 class="text-3xl font-black text-slate-900 mb-4 tracking-tighter uppercase leading-none">Belum Ada Data Armada</h3>
                        <p class="text-slate-500 font-bold mb-12 max-w-sm mx-auto text-sm italic tracking-tight italic">Tambahkan data mobil pertama Anda ke dalam sistem manajemen sekarang.</p>
                        <a href="{{ route('admin.cars.create') }}"
